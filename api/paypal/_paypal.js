...
function orderItemsHtml(items) {
  ...
  return items.map((item) => {
    ...
    return `<div style="margin:0 0 12px;padding:13px 14px;border:1px solid #e7dfd0;border-radius:14px;background:#fffdf8;color:#25332a"><strong style="display:block;margin-bottom:7px">${escapeHtml(item.title || "Produkt")}</strong>${details ? `<div style="margin-bottom:8px;color:#637064;font-size:13px;line-height:1.5">${details}</div>` : ""}<div style="display:block;color:#556b5d;font-size:14px;line-height:1.65">Menge: ${escapeHtml(item.quantity || 1)}<br>Einzelpreis: ${escapeHtml(formatEuro(item.unit_price))}<br><strong style="color:#25332a">Positionssumme: ${escapeHtml(formatEuro(item.line_total))}</strong></div></div>`;
  }).join("");
}
