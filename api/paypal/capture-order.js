import { assertPaymentEnabled, captureMatchesOrder, getCustomerOrderSummary, getSupabaseAdmin, paypalRequest, sendAdminOrderNotification, sendCustomerOrderConfirmation, sendJson } from "./_paypal.js";
export default async function handler(request, response) {
  ...
  try {
    ...
    if (order.payment_status === "paid") {
      await sendAdminOrderNotification(supabase, order.id);
      const customerConfirmationSent = await sendCustomerOrderConfirmation(supabase, order.id);
      return sendJson(response, 200, { ok: true, alreadyCaptured: true, customerConfirmationSent, order: await getCustomerOrderSummary(supabase, order.id) });
    }
    ...
    const customerConfirmationSent = await sendCustomerOrderConfirmation(supabase, order.id, captureTime || new Date());
    return sendJson(response, 200, { ok: true, customerConfirmationSent, order: await getCustomerOrderSummary(supabase, order.id) });
